...
import { useGetMyPublishedTriplatesQuery, useGetUserByUserIdQuery } from '@/redux/services/firestore'
...
const Mypage = () => {
  ...
  const { data: userData, isLoading: isUserLoading } = useGetUserByUserIdQuery(userId)
  const { data: triplatesData, isLoading: isTriplatesLoading } = useGetMyPublishedTriplatesQuery(currentUser.uid)
            ...
            <div css={styles.layoutCardTriplate}>
              {
                triplatesData.map((list) => (
                  <>
                    ...
                    <div>{list.tags?.map((tag) => (<><span>{tag}</span></>))}</div>
                    <div>----------------------------------------------------------------</div>
                  </>
                ))
              }
